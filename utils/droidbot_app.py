...
class DroidBotAppConn(Adapter):
    ...
    def __init__(self, device=None,adb_clt_path="adb"):
        """
    ...
    def connect(self):
        ...
        try:
            ...
            forward_cmd = f"{self.adb_clt_path} %s forward tcp:%d %s" % (serial_cmd, self.port, DROIDBOT_APP_REMOTE_ADDR)
            print(forward_cmd)
    ...
    def disconnect(self):
        ...
        if self.sock is not None:
            try:
                ...
                print(e)
        try:
            forward_remove_cmd = f"{self.adb_clt_path} -s %s forward --remove tcp:%d" % (self.device.serial, self.port)
    ...
    def get_views(self):
        ...
        # print(f"self.last_acc_event in droidbot_app.get_views is {self.last_acc_event}", self.last_acc_event)
        ...
        # print(f"view_tree in droidbot_app.get_views is {view_tree}", view_tree)
        if not view_tree:
            return None
        view_tree['parent'] = -1
        view_list = []
        ...
        # print(f"view_list in droidbot_app.get_views is {view_list}", view_list)
        return view_list
