...
class ADB(Adapter):
    ...
    def __init__(self, device=None, adb_clt_path="adb"):
        ...
        self.cmd_prefix = [f"{self.adb_clt_path}", "-s", device.serial]
    ...
    def run_cmd(self, extra_args):
        ...
        args += extra_args
        try:
            self.logger.debug('执行命令:')
            self.logger.debug(args)
            r = subprocess.check_output(args).strip()
            if not isinstance(r, str):
                r = r.decode()
            # self.logger.debug('返回结果:')
            # self.logger.debug(r)
    ...
    def touch(self, x, y, orientation=-1, event_type=DOWN_AND_UP):
        ...
        # adb_cmd = "input tap %d %d" % self.__transform_point_by_orientation((x, y), orientation, self.get_orientation())
        # self.logger.info("for the touch -- executing ADB Command: %s" % adb_cmd)
    ...
    def drag(self, start_xy, end_xy, duration, orientation=-1):
        ...
        if version <= 15:
            ...
            # adb_cmd = "input swipe %d %d %d %d" % (x0, y0, x1, y1)
            self.shell("input swipe %d %d %d %d" % (x0, y0, x1, y1))
        else:
            # adb_cmd = "input touchscreen swipe %d %d %d %d %d" % (x0, y0, x1, y1, duration)
            self.shell("input touchscreen swipe %d %d %d %d %d" % (x0, y0, x1, y1, duration))
        # self.logger.info("For the longtouch -- executing ADB Command: %s" % adb_cmd)
        # self.shell(adb_cmd)
    ...
    def enter(self):
        return self.shell("input keyevent KEYCODE_ENTER")  
    
    def click(self,x,y):
        return self.shell("input tap %d %d" % (x,y))

    def swipe(self, x1, y1, x2, y2, duration=1500):
        return self.shell(f"input swipe {x1} {y1} {x2} {y2} {duration}")
    
    def uninstall_app(self,app_package):
        return self.shell(f"pm uninstall {app_package}")
